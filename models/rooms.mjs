...
// async function trackOpenRooms(id, remove = false) {
//     const roomKey = (roomId) => `openroom:${roomId}`; // Consistent and descriptive key format
  
//     const operations = [];
//     if (remove) {
//       operations.push(db.sRem("openrooms", roomKey(id))); // Remove room from open rooms set
//     } else {
//       operations.push(db.sAdd("openrooms", roomKey(id))); // Add room to open rooms set
//       operations.push(db.expire(roomKey(id), process.env.EXPIRE_ROOMS || 86400)); // Set expiration for open room key
//     }
//     operations.push(db.set("rooms:lastmodified", new Date().toUTCString())); // Update last modified timestamp
  
//     try {
//       await Promise.all(operations);
//     } catch (error) {
//       console.error("Error tracking open rooms:", error);
//       // Handle errors appropriately, e.g., retry or log for debugging
//     }
//   }
